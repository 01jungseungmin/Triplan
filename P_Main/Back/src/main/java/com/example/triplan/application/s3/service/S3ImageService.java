package com.example.triplan.application.s3.service;
...
public class S3ImageService {
    ...
    public List<String> uploadImages(List<MultipartFile> images) throws S3Exception {
        ...
        return images.stream()
                .map(image -> {
                    try {
                        return this.upload(image); // 업로드 로직 호출
                    } catch (S3Exception e) {
                        throw new RuntimeException(e); // RuntimeException으로 래핑
                    }
                })
                .toList(); // 업로드된 URL 리스트 반환
    }
