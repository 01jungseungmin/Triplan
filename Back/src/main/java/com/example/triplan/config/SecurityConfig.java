    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .authorizeHttpRequests((authorize) -> authorize
                        // 로그인, 회원가입 등 인증 없이 허용
                        .requestMatchers("/", "/index.html","/swagger-ui/**","/{boardId}/answer" ,"/v3/api-docs/**","/login", "/join", "/main", "/api/load","/api/boards","/place/findAll", "/place/details/{placeId}", "/api/boards/{boardId}").permitAll() // 로그인, 회원가입, 메인 페이지는 모두 접근 가능                        .requestMatchers("/admin/**").hasRole("ADMIN")
